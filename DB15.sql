...
CREATE TABLE Accounts (
    ...
    password NVARCHAR(255) NULL,               -- null if Google
    name NVARCHAR(255) NULL,
    picture NVARCHAR(500) NULL,
	role NVARCHAR(20) CHECK (role IN ('learner','instructor','admin')),  
	workplace NVARCHAR(255) NULL,              --
    phone NVARCHAR(20) NULL,                  
    dateOfBirth nvarchar(50) NULL,                     -- Ngày sinh
    gender NVARCHAR(10) CHECK (gender IN (N'Nam', N'Nữ', N'Khác')) NULL, --
    address NVARCHAR(255) NULL   ,    
	isActive BIT DEFAULT 1
);

--tạo bảng mới InstructorRequest :
CREATE TABLE InstructorRequest (
    requestID INT IDENTITY(1,1) PRIMARY KEY,
    accountID INT NOT NULL unique,
    fullName NVARCHAR(100), -- lấy từ bảng account hoặc ko
    email NVARCHAR(100),-- lấy từ bảng account hoặc ko
    phone NVARCHAR(20),  -- lấy từ bảng account hoặc ko
    title NVARCHAR(100), -- chức danh nghề nghiệp
...
GO
-- ============================================
-- TRIGGER: Notify learner when enrollment success
-- ============================================
CREATE TRIGGER trg_Enrollment_SuccessNotify
ON Enrollments
AFTER INSERT
AS
BEGIN
    ...
    SELECT 
        ...
        CONCAT(N'Bạn đã đăng ký thành công khóa học "', c.Title, N'".')
    FROM inserted i
    JOIN Courses c ON i.CourseID = c.CourseID;
END;
GO
-- ============================================
-- TRIGGER: Notify admin when new report submitted
-- ============================================
CREATE TRIGGER trg_Report_NewNotifyAdmin
ON Reports
AFTER INSERT
AS
BEGIN
    ...
    SELECT 
        a.AccountID,
        'System',
        N'Báo cáo mới từ người dùng',
        CONCAT(N'Người dùng "', acc.name, N'" đã gửi báo cáo: "', LEFT(r.Title, 100), N'".')
    FROM inserted r
    CROSS JOIN Accounts a
    JOIN Accounts acc ON acc.AccountID = r.AccountID
    WHERE a.role = 'admin';
END;
GO
